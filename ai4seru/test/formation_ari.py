...
if __name__ == '__main__':
    ...
    for _ in range(pop_num):
        formation_code = Initialization.initial_formation_code(config_seru.num_of_workers)
        formation = SeruFormation(formation_code=formation_code)
        Initialization.produce_seru_formation(config_seru.num_of_workers, formation)
        PSF.append(formation)
        solution: Solution = Solution(formation=formation, schedule=schedule)  # Solution 类型
        CalculateFitnessEDD.calculate_fitness(solution, config_seru)

    # 假设 solutions 是 List[Solution]
    ari_list = calculate_ari(PSF)
    ari_list_sorted = sorted(ari_list, reverse=True)
    print(ari_list)  # 每个 formation 的“平均一致性”得分
