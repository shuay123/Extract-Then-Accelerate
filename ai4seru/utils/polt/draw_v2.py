...
def _normalize_truncate_mode(truncate: str) -> str:
    """Normalize truncate mode aliases.
    - 'min_len' : cut by minimum number of points (per-run)
    - 'min_end' : cut by minimum end time (per-run + mean)
    - 'overlap' : cut by time overlap window
    Aliases:
      'min_time'/'mintime' -> 'min_end'
      'min_points'/'minpts' -> 'min_len'
    """
    t = (truncate or "none").lower()
    if t in {"min_time", "mintime"}:
        return "min_end"
    if t in {"min_points", "minpts"}:
        return "min_len"
    return t
...
def plot_multi_xy_groups(
    ...
    *,
    ...
    for i in range(runs_to_plot):
        ...
        t_mode = _normalize_truncate_mode(truncate)
        t_start = None
        t_end = None
        l_min = None
...
def plot_multi_xy_groups_mintime(
    groups: Union[Sequence[GroupSeries], Dict[str, GroupSeries]],
    labels: Optional[Sequence[str]] = None,
    *,
    truncate: str = "min_end",
    **kwargs,
) -> Dict:
    """便捷封装：按“最短结束时间”截断（truncate='min_end'）。

    适用场景：你认为不同组的对比 run “运行总时长应该一致”，希望每个子图中两条线在同一时间范围内结束，
    就用这个封装（或直接传 truncate='min_end' / truncate='min_time'）。
    """
    return plot_multi_xy_groups(groups, labels=labels, truncate=truncate, **kwargs)
