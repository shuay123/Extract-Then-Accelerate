...
def _coerce_group_to_runs(g: GroupSeries, name: str) -> List[np.ndarray]:
    """把 group 规范成 List[np.ndarray]，每个 run 都是 shape=(L,2+) 的二维数组。

    支持输入：
    - 单条 run：np.ndarray (L,2+) 或 点列表 [[y,x],[y,x],...]
    - 多条 run：list/tuple/Sequence of runs，其中每个 run 可是 (L,2+) ndarray 或点列表
    - ndarray (R,L,2+) / ndarray(object) (R,) 也支持（会拆成 R 条 run）
    """
    if g is None:
    ...
    if isinstance(g, np.ndarray):
        if g.ndim == 2:
            ...
            if arr.shape[1] < 2:
                raise ValueError(f"{name} 必须是 shape=(L,2+) 的二维数组，但 got {arr.shape}")
            return [arr]

        if g.ndim == 3:
            if g.shape[-1] < 2:
                raise ValueError(f"{name} 必须是 shape=(R,L,2+) 的三维数组，但 got {g.shape}")
            return [np.asarray(g[i], dtype=float) for i in range(g.shape[0])]

        # 允许单点 run: shape=(2,) => (1,2)
        if g.ndim == 1 and g.shape[0] >= 2 and g.dtype != object:
            arr = np.asarray(g, dtype=float).reshape(1, -1)
            if arr.shape[1] < 2:
                raise ValueError(f"{name} 必须至少包含 2 列 [y,x]，但 got {arr.shape}")
            return [arr]

        # object array (R,) containing runs
        if g.ndim == 1 and g.dtype == object:
            g = list(g)  # fallthrough to sequence handling
        else:
            raise ValueError(f"{name} 必须是 (L,2+) / (R,L,2+) / 点列表 / run 列表，但 got ndarray.ndim={g.ndim}")

    # -------- sequence cases (list/tuple/other sequences) --------
    if isinstance(g, ABCSequence) and not isinstance(g, (str, bytes)):
        if len(g) == 0:
            return []

        def _is_scalar(z) -> bool:
            return isinstance(z, (int, float, np.integer, np.floating)) and not isinstance(z, (bool, np.bool_))

        def _is_point(p) -> bool:
            # point: (2,) like [y,x] or np.array([y,x])
            if isinstance(p, np.ndarray):
                return p.ndim == 1 and p.shape[0] >= 2 and _is_scalar(p[0]) and _is_scalar(p[1])
            if isinstance(p, (list, tuple)) and len(p) >= 2:
                return _is_scalar(p[0]) and _is_scalar(p[1])
            return False

        # case A: g is a single run represented as list of points
        if all(_is_point(e) for e in g):
            arr = np.asarray(g, dtype=float)
            if arr.ndim != 2 or arr.shape[1] < 2:
                raise ValueError(f"{name} 必须能转换为 shape=(L,2+) 的二维数组，但 got {arr.shape}")
            return [arr]

        # case B: g is a list of runs (ragged lengths allowed)
        runs: List[np.ndarray] = []
        for i, item in enumerate(g):
            arr = np.asarray(item, dtype=float)

            # allow single-point run: (2,) -> (1,2)
            if arr.ndim == 1 and arr.shape[0] >= 2:
                arr = arr.reshape(1, -1)

            if arr.ndim != 2 or arr.shape[1] < 2:
                raise ValueError(f"{name}[{i}] 必须是 shape=(L,2+) 的二维数组（或点列表），但 got {arr.shape}")

            runs.append(arr)
        return runs

    # -------- fallback: try interpret as a single 2D run --------
    arr = np.asarray(g, dtype=float)
    ...
    if arr.ndim == 1 and arr.shape[0] >= 2:
        ...
        if arr.shape[1] >= 2:
            return [arr]
    raise ValueError(f"{name} 必须是二维点序列(或其列表)，但 got {type(g)}")
...
def plot_multi_xy_groups(
    ...
    *,
    ...
    truncate:
      - 'none'    : 不截断
      - 'min_len' : per-run 子图按各组该 run 的最短点数截断（按时间排序后取前 L_min 个点）；均值图仍按最短结束时间截断
      - 'min_end' : per-run 子图按该 run 在各组中的最短结束时间截断；均值图按各组内最短 run 的结束时间做全局截断
      - 'overlap' : 使用各组时间范围交集 [max(start), min(end)]
    ...
    for i in range(runs_to_plot):
        ...
        # truncate: 同一子图里，不同组的第 i 条 run 长度可能不同。
        # - min_len: 取各组该 run 的最短点数 L_min（按时间排序后取前 L_min 个点）
        # - min_end/overlap: 按最短结束时间(min_end)或时间交集(overlap)截断
        t_mode = (truncate or 'none').lower()
        t_start = None
        t_end = None
        l_min = None
        ...
        if t_mode == 'min_len':
            ...
            for grp in runs_xy:
                if i < len(grp):
                    xx = grp[i][0]
                    if xx.size > 0:
                        lens.append(int(xx.size))
            if lens:
                l_min = int(min(lens))

        elif t_mode in {'min_end', 'overlap'}:
            starts = []
            ends = []
            for grp in runs_xy:
                if i < len(grp):
                    xx = grp[i][0]
                    if xx.size > 0:
                        starts.append(float(xx[0]))
                        ends.append(float(xx[-1]))
            if ends:
                t_end = float(min(ends))
                if t_mode == 'overlap' and starts:
        ...
        for lab, group in zip(_labels, runs_xy):
            ...
            if x.size == 0:
                continue
            if l_min is not None and x.size > l_min:
                x = x[:l_min]
                y = y[:l_min]
    ...
    # truncate mean-plot time range to the 'shortest' (per your request)
    t_mode = (truncate or 'none').lower()
    if t_mode in {'min_len', 'min_end', 'overlap'}:
        group_starts = []
        group_ends = []
        for grp in runs_xy:
            xs = [x for (x, _y) in grp if x.size > 0]
            if not xs:
                continue
            starts = [float(x[0]) for x in xs]
            ends = [float(x[-1]) for x in xs]
            # 组内最短长度：最小结束时间
            group_ends.append(min(ends))
            if t_mode == 'overlap':
                group_starts.append(max(starts))
            else:
                group_starts.append(min(starts))
        if group_ends:
            x_max2 = min(group_ends)
            x_min2 = max(group_starts) if group_starts else x_min
            if x_min2 < x_max2:
                x_min, x_max = float(x_min2), float(x_max2)
            else:
                # 如果没有公共起点，也至少按最短结束时间截断
                x_max = float(x_max2)
    ...
    def _make_grid() -> np.ndarray:
        ...
        # union 候选（若已 truncate，则只取截断区间内的 x）
        ax = all_x[~np.isnan(all_x)]
        ax = ax[(ax >= x_min) & (ax <= x_max)]
        uniq = np.unique(ax)
        uniq = np.sort(uniq)
...
def plot_multi_xy_groups_minlen(
    groups: Union[Sequence[GroupSeries], Dict[str, GroupSeries]],
    labels: Optional[Sequence[str]] = None,
    *,
    truncate: str = "min_len",
    **kwargs,
) -> Dict:
