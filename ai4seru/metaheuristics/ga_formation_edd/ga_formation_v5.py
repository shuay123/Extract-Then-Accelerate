...
class GA:
    ...
    def run(self, ) -> (SeruFormation, SeruSchedule, Solution,List[Solution]):
        ...
        while time.time() - start_time < self.config_ga.max_runtime:
            ...
            # 1. 精英保留 (Elitism): 只保留父代中前 10% 的最强个体
            # 这保证了历史最优解不会丢失 (兜底策略)
            PSF.sort(key=lambda x: x.fitness) # 升序，Makespan越小越好
            num_elites = max(1, int(self.config_ga.population_size * 0.1)) 
            elites = [p.__copy__() for p in PSF[:num_elites]]
            
            # 2. 子代上位 (Generational): 剩余 90% 的位置强制由子代填充
            # 即使子代目前比父代差，也强迫它们上位，这样算法才能跳出当前的局部最优坑位
            offspring.sort(key=lambda x: x.fitness)
            num_offspring_needed = self.config_ga.population_size - num_elites
            
            # 选取子代中最好的来填补 (如果子代不够，逻辑上由下面步骤3兜底，通常够)
            next_generation = elites + offspring[:num_offspring_needed]
            
            # 3. 数量补齐 (Safety Check): 万一 offspring 数量不够，用旧父代凑数
            if len(next_generation) < self.config_ga.population_size:
                needed = self.config_ga.population_size - len(next_generation)
                # 从剩下的父代中补齐
                next_generation += PSF[num_elites : num_elites + needed]
            
            # 4. 更新种群
            PSF = next_generation
            
            # 5. 重新排序，确保 PSF 有序，方便后续的多样性检查和局部搜索
            ...
            if PSF[0].fitness == PSF[int(self.config_ga.population_size * 0.5)].fitness:
                # print("  >> [多样性保护] 触发强制变异...")
                for k in range(int(self.config_ga.population_size * 0.5), self.config_ga.population_size):
                    # 随机交换变异
