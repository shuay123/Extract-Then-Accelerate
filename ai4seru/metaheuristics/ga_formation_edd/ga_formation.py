...
class GA:
    ...
    def run(self, ) -> (SeruFormation, SeruSchedule, Solution,List[Solution]):
        ...
        while time.time() - start_time < self.config_ga.max_runtime:
            ...
            best_formation: SeruFormation = min(PSF + offspring + [best_solution.formation], key=lambda x: x.fitness).__copy__()  # SeruFormation 类型

            PSF = offspring  # List[SeruFormation]
            # 对 PSF 列表根据 fitness 属性值从大到小进行排序
            PSF.sort(key=lambda x: x.fitness, reverse=True)

            # 打印排序后的 PSF 列表，验证排序结果
            # PSF[0]=best_formation.__copy__()

            # 局部搜索：优化最佳构造
            best_formation = GaOperator.local_search_formation_edd(formation=best_formation, best_scheduling=schedule, config_ccea=self.config_ga, config_seru=self.config_seru)

            current_best_solution = Solution(formation=best_formation.__copy__(), schedule=schedule)
