...
from coeff import CalculateFitness
...
class ConflictModel:
    ...
    def compute_r(self, formation: SeruFormation, schedule: SeruSchedule, 
                  ...
                  batch_mode: str = "base") -> Tuple[float, float, float]:
            ...
            for p in bset:
                ...
                for q, c_pq in self.batch_adj[p]:
                    ...
                    if batch_mode == "base":
                        ...
                        tp = b2p.get(p, {}).get('产品类型')
                        tq = b2p.get(q, {}).get('产品类型')
...
def _ensure_one_based_square_matrix(M, size: int) -> np.ndarray:
    """
    统一成 (size+1, size+1) 的 1-based 矩阵：索引 1..size 有效，0 行/列占位为 0。
    兼容输入：
      - size x size （0-based）
      - (size+1) x (size+1)（1-based）
      - 其它形状：会做 padding/裁剪到目标尺寸
    """
    ...
    if A.shape == target:
        ...
        out = np.zeros(target, dtype=float)
        r = min(target[0], A.shape[0])
        c = min(target[1], A.shape[1])
        out[:r, :c] = A[:r, :c]

        # 如果用户传的是 size x size，但被 padding 逻辑放到了 [0:size,0:size]，这里做一次“平移修正”
        # 仅当右下角明显空、左上角有值时触发（尽量不误伤）
        if A.shape[0] == size and A.shape[1] == size and np.allclose(out[1:, 1:], 0) and not np.allclose(out[:size, :size], 0):
            tmp = out.copy()
            out[:] = 0.0
            out[1:, 1:] = tmp[:size, :size]

    # clip + 对称化 + 对角线置 0（ConflictModel 里也会做，但这里先保证输入干净）
...
def _scores_to_conf_matrix(edge_scores, size: int) -> np.ndarray:
    """
    edge_scores: 兼容度分数矩阵（越低越差）
    return: 冲突矩阵（越大越冲突/惩罚）
    """
    S = _ensure_one_based_square_matrix(edge_scores, size)
    C = 1.0 - S
    C = np.clip(C, 0.0, 1.0)
...
class CCEA:
    def __init__(self, worker_map=None, batch_map=None, edge_scores_worker=None, edge_scores_batch=None):
        ...
        # 2) 挂到 config_seru（calculate_fitness.py 会用到）
        if self.worker_map:
            self.config_seru.worker_map = self.worker_map
            # 强烈建议同步逻辑规模，否则初始化/编码长度会不匹配
        ...
        # 3) 如你后面要接 GNN edge_scores，这里也可以顺便保存
        ...
        if edge_scores_worker is not None:
            worker_conf = _scores_to_conf_matrix(edge_scores_worker, W)
        else:
            worker_conf = _load_conf_matrix(wp, W, one_based=True)

        if edge_scores_batch is not None:
            batch_conf = _scores_to_conf_matrix(edge_scores_batch, B)
        else:
