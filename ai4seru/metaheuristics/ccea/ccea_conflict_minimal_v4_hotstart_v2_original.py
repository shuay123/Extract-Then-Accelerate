...
class ConflictModel:
    ...
    def compute_r(self, formation: SeruFormation, schedule: SeruSchedule,
        ...
        for s_idx, blist in enumerate(seru_batches):
            ...
            # 将当前 Seru 的 batch 集合化，加速查找
            bset = set(int(b) for b in blist)
            
            for p in blist: # 只遍历当前 Seru 里的 batch
                p = int(p)
                if p >= len(self.batch_adj): continue
                
                # 遍历 p 的 Top-K 邻居
                for q, c_pq, logit_val in self.batch_adj[p]:
                    q = int(q)
                    if q <= p: continue # 避免重复计算
                    
                    # 只有当邻居 q 也在同一个 Seru (bset) 中时才计算
                    if q in bset:
                        if batch_mode == "base":
                            P_B += float(c_pq)
                        else:
...
class CCEA:
    ...
    def run(self, Pop_Size) -> Tuple[SeruFormation, SeruSchedule, Solution]:
        ...
        return best_solution.formation, best_solution.schedule, best_solution, cmax_his, fitness_his
