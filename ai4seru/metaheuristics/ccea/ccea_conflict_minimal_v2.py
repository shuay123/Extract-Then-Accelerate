# -*- coding: utf-8 -*-
"""
最终修复版：CCEA + 冲突软约束 (Conflict Soft Constraint)
修复内容：
1. 修正 ExcelDataLoader 初始化判断逻辑，防止因 hasattr 返回 True 但值为 None 导致的数据未加载错误。
2. 包含完整的 ConflictModel 和 CCEA 逻辑。
"""
import sys
import os
import time
...
class ConflictModel:
    def __init__(
        ...
        # 如果没有任何冲突定义，设为1避免除零，此时 r 将始终为 0
        if self.W_tot <= 1e-9:
            self.W_tot = 1.0  
    ...
    def compute_r(self, formation: SeruFormation, schedule: SeruSchedule, excel_loader: ExcelDataLoader, batch_mode: str) -> Tuple[float, float, float]:
        """计算总违背率 r 及分项 PW, PB"""
            ...
            for p in bset:
                ...
                for q, c_pq in self.batch_adj[p]:
                    ...
                    if batch_mode == "base":
                        ...
                        tp = b2p.get(p, {}).get("product_type")
                        tq = b2p.get(q, {}).get("product_type")
...
class CCEA:
    def __init__(self):
        ...
        # [修复点] 强制检查数据字典是否为 None
        # 注意：不要仅用 hasattr，因为属性可能存在但值为 None
        ...
        # 再次检查，防止路径配置错误导致依旧为 None
        if not getattr(self.loader, "batch_to_product_dict", None):
             print("[Warning] batch_to_product_dict is still None after loading. Check excel paths!")
    ...
    def run(self) -> Tuple[SeruFormation, SeruSchedule, Solution]:
        ...
        self._evaluate_solution(curr_sol, batch_mode="dynamic", M_ref=1.0)
        M_ref = float(curr_sol.makespan if curr_sol.makespan > 1e-5 else 1.0)
        
        self._evaluate_solution(curr_sol, batch_mode="dynamic", M_ref=M_ref)
