# -*- coding: utf-8 -*-
"""
Experiment baseline CCEA, instantiated without GNN edge scores.

Implements conflict soft constraints and symmetric worker/batch evaluation.
"""
import sys
import os
import time
...
class ConflictModel:
    def __init__(
        ...
        # Retain the strongest Top-K conflicts in each row.
        top_k_worker: int = 10, 
        top_k_batch: int = 20    
    ):
        self.rho = float(rho)
        ...
        # Truncate the adjacency list to the strongest K conflicts per entity.
        ...
        # Precompute the static logit values.
    ...
    def _build_adj(M: np.ndarray, eps: float, top_k: int) -> List[List[Tuple[int, float]]]:
        ...
        for i in range(n):
            ...
            if len(valid_idxs) > top_k:
                ...
                # Sort the weights selected by valid_idxs.
    ...
    def worker_pair_weight_dynamic(self, seru_idx: int, logit_val: float) -> float:
        """
        使用预计算的 logit_val，避免重复计算 math.pow 和 math.log。
        """
        # 1. 获取上下文
        avg_weakness = 0.5
    ...
    def compute_r(self, formation: SeruFormation, schedule: SeruSchedule,
        ...
        for i, nbrs in enumerate(self.worker_adj):
            # Convert the zero-based index to a one-based ID.
            wid_real_id = i + 1 
        ...
        for s_idx, blist in enumerate(seru_batches):
            ...
            for p_id in blist: 
                p_id = int(p_id)  # One-based ID.
                
                # Convert the ID to an index for adjacency-list access.
                p_idx = p_id - 1
                ...
                for q_idx, c_pq, logit_val in self.batch_adj[p_idx]:
                    ...
                    # Convert the index to an ID before checking membership in bset.
                    q_id = q_idx + 1
...
class CCEA:
    ...
    def _as_numpy_scores(self, M, size: int, name: str) -> np.ndarray:
        ...
        if isinstance(M, dict):
            ...
            # Detect zero-based indexing before conversion.
            ...
            for ki, row in M.items():
                ...
                if isinstance(row, dict):
                    ...
                    # Inner and outer dictionaries use the same indexing convention.
