# -*- coding: utf-8 -*-
"""
最终修复版 V3：CCEA + 冲突软约束 (Symmetric Conflict Update Strategy)
更新策略：
1. 进化 Seru 构造时 (Subject: Workers): Worker Conflict = Dynamic (启用阈值惩罚), Batch Conflict = Base (静态).
2. 进化 Seru 调度时 (Subject: Batches): Worker Conflict = Base (静态), Batch Conflict = Dynamic (启用Weakness+阈值).
"""
import sys
import os
import time
...
class ConflictModel:
    ...
    def worker_pair_weight_dynamic(self, w_raw: float) -> float:
        """
        [新增] 计算工人的动态冲突权重
        """
        w = float(w_raw)
        if self.theta_hard_worker is not None and w >= self.theta_hard_worker:
            return 10.0 * w
        return w
            ...
            for j, w in nbrs:
                ...
                if sj is not None and si == sj:
                    ...
                    if worker_mode == "base":
                        ...
                        P_W += self.worker_pair_weight_dynamic(float(w))
