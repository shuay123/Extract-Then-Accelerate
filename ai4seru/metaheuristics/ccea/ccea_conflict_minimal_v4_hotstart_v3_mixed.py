...
class ConflictModel:
    ...
    def _build_adj_topk(self, M: np.ndarray, eps: float, top_k: int) -> List[List[Tuple[int, float, float]]]:
        ...
        for i in range(n):
            ...
            for j in best_idxs:
                ...
                adj[i].append((int(j), w, logit_val))
        return adj
    ...
    def compute_r(self, formation: SeruFormation, schedule: SeruSchedule,
        ...
        for s_idx, seru in enumerate(serus):
            ...
            # 2. 映射 Batch
            # 注意：Schedule 里的 batch assignment 可能会覆盖 seru.batches_set
            # 这里先看 schedule 对象
            batches_in_seru = []
            if getattr(schedule, "batches_assignment", None):
                if s_idx < len(schedule.batches_assignment):
                    batches_in_seru = schedule.batches_assignment[s_idx]
            else:
                batches_in_seru = getattr(seru, "batches_set", [])
            
            for bid in batches_in_seru:
    ...
    def compute_r2(self, formation: SeruFormation, schedule: SeruSchedule,
                  excel_loader: ExcelDataLoader,
                  worker_mode: str = "base",
                  batch_mode: str = "base",
                  config_seru=None) -> Tuple[float, float, float]:
        ...
        修正点：
        1. 恢复了批次分配的取模 (Modulo) 逻辑，防止批次组多于 Seru 数时被丢弃。
        2. 保持了 Top-K 的高速计算。
        3. 保持了 1-based ID 到 0-based Index 的正确映射。
        """
        serus = getattr(formation, "seru_set", []) or []
        num_serus = len(serus)
        
        # --- 0. 预处理：构建 ID -> SeruIdx 映射 (O(W) + O(B)) ---
        worker_to_seru = {}
        batch_to_seru = {}
        
        # 0.1 映射 Worker & 预计算 Seru Weakness
        seru_weakness_realtime = [] 
        ...
        for s_idx, seru in enumerate(serus):
            ...
            # 实时计算 Weakness (Worker Dynamic 模式)
            if worker_mode == "dynamic":
                wk = self._calculate_single_seru_avg_weakness(wuniq, excel_loader, config_seru)
                seru_weakness_realtime.append(wk)
            else:
        ...
        # 0.2 映射 Batch (修正：恢复取模逻辑)
        # 优先使用 schedule 中的 assignment
        assigned_batches = getattr(schedule, "batches_assignment", None)
        if assigned_batches and num_serus > 0:
            for grp_idx, group in enumerate(assigned_batches):
                # [关键修正] 使用取模，防止 group 数多于 Seru 数时被丢弃
                target_s_idx = grp_idx % num_serus
                for bid in group:
                    batch_to_seru[int(bid)] = target_s_idx
        else:
            # Fallback: 使用 seru.batches_set (通常在初始化或由 assignment 同步后存在)
            for s_idx, seru in enumerate(serus):
                for bid in getattr(seru, "batches_set", []):
                    batch_to_seru[int(bid)] = s_idx
        ...
        for i, nbrs in enumerate(self.worker_adj):
            ...
            if s_i is None: continue # 该工人未分配
...
class CCEA:
    ...
    def _evaluate_solution(self, solution: Solution, 
        ...
        # 2. 计算冲突率 (传入双模式)
        r, P_W, P_B = self.conflict_model.compute_r2(
