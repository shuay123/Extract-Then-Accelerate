...
import com.fasterxml.jackson.databind.ObjectMapper; // 需要 Jackson 库
...
public class CceaBoundService {

    public static double getInitialBound(SeruSamplerService.Result r, SeruSamplerService.Config cfg) {
        try {
            // 1. 准备 config_seru
            String jsonPayload = convertResultToJson(r, cfg);
            ...
            if (json != null) {
                ...
                if (map.containsKey("fitness")) {
                    return Double.parseDouble(map.get("fitness").toString());
                }
            }
        } catch (Exception e) {
            System.out.println("CCEA Warning: " + e.getMessage());
        }
        return -1.0; // 失败标记
    }

    public static String convertResultToJson(SeruSamplerService.Result r, SeruSamplerService.Config cfg) {
        try {
            // ==========================================
            // 1. 构建 config_seru 部分
            // ==========================================
