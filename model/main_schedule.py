...
def main(args):
    ...
    for epoch in range(args.get('n_epochs', 0)):
        ...
        if (epoch + 1) % 2 == 0:
            ...
            if val_metrics['precision'] > best_val_pre:
                ...
                model_name = f"seruschedule_W{args.get('n_batch', 0)}_J{args.get('n_nodes', 0)}_pre.pt"
                torch.save(model.state_dict(), model_name)
                print(f"  ✓ 保存最佳模型 (Prec: {best_val_pre:.4f})")
            
            if val_metrics['f1'] > best_val_f1:
                best_val_f1 = val_metrics['f1']
                model_name = f"seruschedule_W{args.get('n_batch', 0)}_J{args.get('n_nodes', 0)}_f1.pt"
                torch.save(model.state_dict(), model_name)
    ...
    # 绘制训练曲线
    print("\n绘制训练曲线...")
    plot_training_curves(history, save_path=f"training_curves_W{args.get('n_batch', 0)}_J{args.get('n_nodes', 0)}.png")
...
if __name__ == "__main__":
    # args = parse_args()
    args = load_yaml_config('JCompany_W8_J8_5000_schedule.yaml')
