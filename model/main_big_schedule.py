...
from util.DatasetGennerate import ClusteringDataset, MeaningfulClusteringDataset, SemanticClusteringDataset, get_dataset_big_shedule
...
def train_epoch2(model, dataloader, optimizer, device, epoch):
    model.train()
    t_data = 0.0
    t_fwbw = 0.0

    end = time.time()
    for step, (node_features, labels) in enumerate(dataloader):
        t_data += time.time() - end  # dataloader 到手的时间
        ...
        if step == 20:  # 只统计前 20 个 step
            break

    print(f"[Profile] data/step={t_data/21:.3f}s, fwbw/step={t_fwbw/21:.3f}s")
def evaluate(model, dataloader, device):
    ...
    with torch.no_grad():
        for node_features, labels in dataloader:
            ...
            preds = (edge_scores > 0.5).float().view(B, -1)[:, mask_flat]
            labels  = labels.float().view(B, -1)[:, mask_flat]
    ...
    pos_rate = all_labels.float().mean().item()
...
def plot_training_curves(history, save_path='training_curves_W7.png'):
    """绘制训练曲线"""
...
def main(args):
    ...
    train_loader, val_loader, test_loader = get_dataset_big_shedule(args, SizeofDataset = args.get('SizeofDataset', 5000))
    print(f"训练集大小: {len(train_loader.dataset)}")
    print(f"测试集大小: {len(test_loader.dataset)}")
    args_n = args.get('n_nodes', 0) * 1
    # 创建模型
    print("创建模型...")
    ...
    print("验证模型输出的对称性...")
    with torch.no_grad():
        test_input = torch.randn(2, args.get('n_batch', 0), args.get('m', 0) * args_n).to(device)
    ...
    for epoch in range(args.get('n_epochs', 0)):
        ...
        if (epoch + 1) % 1 == 0:
            ...
            print(f"  Dist  - PosRate: {val_metrics['pos_rate']:.4f}, "
                f"PredPosRate: {val_metrics['pred_pos_rate']:.4f}")

            
            # 保存最佳模型
            if val_metrics['precision'] > best_val_pre:
                best_val_pre = val_metrics['precision']
                model_name = f"seruschedule_W{args.get('n_nodes', 0)}_J{args.get('n_batch', 0)}_pre.pt"
                torch.save(model.state_dict(), model_name)
                print(f"  ✓ 保存最佳模型 (Prec: {best_val_pre:.4f})")
            
            if val_metrics['f1'] > best_val_f1:
                best_val_f1 = val_metrics['f1']
                model_name = f"seruschedule_W{args.get('n_nodes', 0)}_J{args.get('n_batch', 0)}_f1.pt"
                torch.save(model.state_dict(), model_name)
...
if __name__ == "__main__":
    # args = parse_args()
    args = load_yaml_config('JCompany_W25_J200_5000.yaml')
